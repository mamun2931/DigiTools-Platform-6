...
const Card = ({ product, selectedProducts, setSelectedProducts }) => {
  ...
  const buySelected = selectedProducts.some((p) => p.id === product.id);
  ...
  return (
    <div>
      <div className="card bg-base-100 shadow-sm hover:hover:scale-105">
        <div className="card-body space-y-5">
          <div className="flex items-center justify-between">
            ...
            <div>
              {tag && (
                <p className={`badge ${currentTagType} font-bold`}>{tag}</p>
              )}
            </div>
          </div>
